...
IGNORED_DIRS = {"output", ".git", "__pycache__", "node_modules", ".next", "_poster_input_images"}
    ...
    for directory in candidate_directories(workspace):
        ...
        for path in entries:
            ...
            if key in seen:
                continue
            seen.add(key)
            found.append((path, file_type, directory))
    return found
...
def collect_paper_candidates(workspace: Path):
    ...
    for root in candidate_scan_roots(workspace):
        for path in iter_workspace_files(root):
            file_type = detect_file_type(path)
            if file_type:
                detected.append((path, file_type, root))
    return detected
...
def copy_images_to_safe_names(workspace: Path, paper_src: Path):
    copied = []
    seen = set()
    image_files = []
    target_dir = workspace / "_poster_input_images"
    target_dir.mkdir(parents=True, exist_ok=True)
    scan_dirs = []
    paper_parent = paper_src.resolve().parent
    for candidate in [paper_parent, paper_parent / "images", paper_parent / "figures", paper_parent / "assets"]:
    ...
    for idx, src in enumerate(sorted(image_files, key=lambda p: p.name.lower()), start=1):
        target = target_dir / f"image_{idx:02d}{src.suffix.lower()}"
...
def main():
    ...
    safe_images = copy_images_to_safe_names(workspace, paper_src)
