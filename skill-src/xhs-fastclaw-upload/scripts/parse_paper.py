...
def split_paragraphs(text: str):
    ...
    blocks = re.split(r"\n{2,}", normalized)
    return [item.strip() for item in blocks if item.strip()]
...
def is_metadata_line(text: str) -> bool:
    value = text.strip()
    return any(value.startswith(prefix + "：") or value.startswith(prefix + ":") for prefix in METADATA_PREFIXES)


def strip_cover_metadata(paragraphs):
    cleaned = []
...
def infer_title(paragraphs, fallback_name: str) -> str:
    for item in paragraphs[:20]:
        ...
        if is_metadata_line(value):
            continue
        if value in {"摘要", "Abstract", "ABSTRACT"}:
            continue
        if 8 <= len(value) <= 120:
            return value
...
def infer_abstract(paragraphs, language: str):
    for idx, item in enumerate(paragraphs[:30]):
        lower = item.lower().strip(":：")
        if language == "zh-CN":
            if lower == "摘要":
                return paragraphs[idx + 1] if idx + 1 < len(paragraphs) else ""
            ...
            if lower.startswith("abstract"):
                ...
                return cleaned or (paragraphs[idx + 1] if idx + 1 < len(paragraphs) else "")
    for item in paragraphs[1:10]:
        if 120 <= len(item) <= 2400 and not is_metadata_line(item):
            return item
    return ""
...
def infer_highlights(paragraphs, abstract_text: str):
    ...
    for item in paragraphs:
        ...
        if not cleaned or cleaned == abstract_text or is_metadata_line(cleaned):
            continue
        if 20 <= len(cleaned) <= 180 and cleaned not in items:
            items.append(cleaned)
        if len(items) >= 4:
            break

    if not items and abstract_text:
        sentences = re.split(r"(?<=[。！？!?])\s*", abstract_text)
        for sentence in sentences:
            cleaned = sentence.strip()
            if 20 <= len(cleaned) <= 120 and cleaned not in items:
                items.append(cleaned)
            if len(items) >= 4:
                break
    return items
...
def detect_sections(text: str):
    ...
    for pos, (start, key) in enumerate(headers):
        ...
        body_lines = [line for line in lines[start + 1 : end] if line and not is_metadata_line(line)]
...
def build_result(metadata: dict, raw_text: str):
    ...
    paragraphs = strip_cover_metadata(split_paragraphs(normalized))
    filtered_text = "\n\n".join(paragraphs)
    language = detect_language(filtered_text or normalized)
    title = infer_title(paragraphs, metadata["paper_safe"])
    abstract_text = infer_abstract(paragraphs, language)
    sections = detect_sections(filtered_text or normalized)
    ...
    return {
        ...
        "body_excerpt": (filtered_text or normalized)[:5000],
        "text_length": len(filtered_text or normalized),
    }, (filtered_text or normalized)
