...
IGNORED_NAMES = {
    ...
    "manual-workspace-inputs.json",
    "parsed-paper.json",
    "paper-body.txt",
    "xiaohongshu-package.json",
    "xiaohongshu-package.docx",
    "xiaohongshu-preview.html",
    "xiaohongshu-result.json",
    "xiaohongshu-post.md",
    "title-options.md",
    "cover-copy.md",
    "hashtags.md",
...
def build_scan_report(workspace: Path):
    ...
    return {
        ...
        "other_files": [str(p.resolve()) for p in sorted(other_files)[:20]],
        "hint": "If the attachment bubble shows a file but this list is empty, the file did not land in the current FastClaw workspace/session.",
    }
