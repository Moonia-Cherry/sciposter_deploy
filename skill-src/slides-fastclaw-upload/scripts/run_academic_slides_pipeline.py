...
def write_progress(workspace_dir, stages, active_index, note, extra=None):
    todo_path = workspace_dir / "todo.md"
    progress_path = workspace_dir / "output" / "slides-progress.json"
...
def run_normalization_with_retry(args, skill_dir, workspace_dir, output_dir, stages):
    normalize_cmd = [
        args.python,
...
def main():
    parser = argparse.ArgumentParser(description="Run the group-meeting slides pipeline end-to-end.")
    parser.add_argument("--workspace", default=".", help="Workspace directory.")
    parser.add_argument("--input", default="", help="Path to parsed-paper.json.")
    ...
    parser.add_argument("--node", default=shutil.which("node") or "", help="Node.js executable path.")
    parser.add_argument("--style", default="classic-blue", help="Slide style request.")
    parser.add_argument("--language", default="zh-CN", help="Output language, such as zh-CN or en.")
    args = parser.parse_args()

    if not args.node:
        parser.error("未找到 Node.js。请使用 SciPoster 部署包携带的 portable Node，或通过 --node 指定路径。")
    ...
    write_progress(workspace_dir, stages, 0, "开始处理上传论文，准备生成组会 PPT。")
    ...
    write_progress(workspace_dir, stages, 2, "正在构建有头有尾的多页组会汇报结构。")
    package_cmd = [
        args.python,
        str(skill_dir / "build_slides_package.py"),
        "--input",
        str(parsed_input),
        "--output-dir",
        str(output_dir),
    ...
    preview_png_path = output_dir / "slides-preview.png"
    preview_html_path = output_dir / "slides-preview.html"

    write_progress(
        workspace_dir,
        stages,
        3,
        "正在先生成逐页预览图和预览页面，用户可以更早看到 PPT 样式。",
        {"package": str(package_path)},
    )
    png_cmd = [
        args.python,
        str(skill_dir / "render_slides_preview_png.py"),
        "--input",
        str(package_path),
        "--output",
    ...
    write_progress(
        ...
        stages,
        4,
        "预览已经生成，正在导出可编辑 PPTX 文件。",
        {"preview_png": str(preview_png_path), "preview_html": str(preview_html_path)},
    )
    render_cmd = [
        args.node,
        str(skill_dir / "render_slides_ppt.cjs"),
        "--input",
        str(package_path),
        "--output",
        str(pptx_path),
    ...
    write_progress(workspace_dir, stages, 5, "正在整理最终下载项和回复内容。")
    report_cmd = [
        args.python,
    ...
    write_progress(
        ...
        stages,
        len(stages),
        "组会 PPT 已生成完成，预览页、逐页图片与可编辑 PPTX 均可使用。",
        {
            "pptx": str(pptx_path),
